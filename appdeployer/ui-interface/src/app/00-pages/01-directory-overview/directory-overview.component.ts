...
import {DialogBoxFunctions} from "../../01-fragement/views/dialog-box/dialog-box.types";
import {DialogBoxComponent} from "../../01-fragement/views/dialog-box/dialog-box.component";
import {RouterOutlet} from "@angular/router";

@Component({
  standalone: true,
  imports: [JavaLocaldatetimeFormatterPipe, KeyValueDialogBoxComponent, SelectableRowHighlighterDirective, DialogBoxComponent, RouterOutlet],
...
export class DirectoryOverviewComponent extends BaseComponent implements AfterViewInit {
  ...
  readonly dialogBoxFunctions: DialogBoxFunctions = {
    open: new Subject<void>(),
    setTitle: new BehaviorSubject<string>('')
  };
  ...
  createDir() {
    this.router.navigate(['directory/create']).then(s => {
      this.dialogBoxFunctions.open.next();
    });
  }

  createFile() {
    this.router.navigate(['file/create']).then(s => {
      this.dialogBoxFunctions.open.next();
    });
  }
}
